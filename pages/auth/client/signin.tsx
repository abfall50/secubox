...
import { useSecubox } from "../../../components/SecuboxContext"
...
const Signin = () => {
	...
	const onSubmit = handleSubmit(async (data) => {
		...
		await fetch("/api/user/connectedTo", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ secubox_id: secubox?.secubox_id, email: data.email })
		}).then(res => res.json())
			...
			<div className="w-4/5 h-[700px] bg-white rounded-3xl shadow-2xl shadow-black flex flex-col lg:w-1/3 xl:w-1/4">
				...
				<div className="w-full h-1/6 flex">
					<div className="w-1/2 h-full">
						<Anchor href="/auth/client/signin" className="w-full h-full flex justify-end items-center text-2xl underline text-yellow-500 font-bold text-center">Se connecter</Anchor>
					</div>
					<div className="w-1/2 h-full">
						<Anchor href="/auth/client/signup" className="w-full h-full flex justify-center items-center text-xl hover:text-2xl hover:underline">S&apos;inscrire</Anchor>
					</div>
				</div>
				...
				<div className="w-full flex justify-center">
					<Anchor href="/forgot-password?back=client" className="underline font-medium">Mot de passe oublié?</Anchor>
				</div>
			</div>
		</div>
	)
